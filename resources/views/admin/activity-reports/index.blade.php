...
@section('content')
    <div class="max-w-7xl mx-auto space-y-6">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                <h1 class="text-2xl font-semibold text-slate-900">Laporan Kegiatan</h1>
                <p class="text-sm text-slate-500">Kelola artikel dan dokumentasi kegiatan.</p>
            </div>
            <a href="{{ route('admin.activity-reports.create') }}"
                    ...
                    <thead>
                        <tr class="bg-slate-50/50 border-b border-slate-200">
                            <th class="px-6 py-3 text-xs font-semibold text-slate-500 uppercase tracking-tight">Judul
                                Artikel</th>
                            <th class="px-6 py-3 text-xs font-semibold text-slate-500 uppercase tracking-tight">Program
                                Kerja</th>
                            ...
                            <tr class="group hover:bg-slate-50/40 transition-colors">
                                <td class="px-6 py-4">
                                    <div class="flex flex-col">
                                        <span class="font-semibold text-slate-900 text-sm">{{ $report->title }}</span>
                                ...
                                <td class="px-6 py-4">
                                    <span
                                        ...
                                        {{ $report->workProgram->name ?? 'Deleted Program' }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 text-center">
                                    <span
                                        class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold border capitalize {{ $report->status == 'published' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-slate-50 text-slate-600 border-slate-200' }}">
                                        {{ $report->status }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 text-right">
                                    <div class="flex items-center justify-end gap-1">
                                        <a href="{{ route('admin.activity-reports.edit', $report) }}"
                                            class="p-1.5 text-slate-400 hover:text-blue-600 rounded-md hover:bg-slate-100 transition-colors"
                                            title="Edit">
                                            <i class="fas fa-edit text-xs"></i>
                                        </a>
                                        <form action="{{ route('admin.activity-reports.destroy', $report) }}" method="POST"
                        ...
                        @empty
                            <tr>
                                <td colspan="4" class="px-6 py-12 text-center">
                                    <p class="text-slate-400 text-sm italic">Belum ada laporan kegiatan.</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
