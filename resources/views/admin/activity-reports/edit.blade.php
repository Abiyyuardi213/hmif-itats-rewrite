...
@section('title', 'Edit Artikel Kegiatan')
...
@section('content')
    <div class="max-w-7xl mx-auto space-y-6">
        ...
        <form action="{{ route('admin.activity-reports.update', $activityReport) }}" method="POST"
                ...
                <div class="lg:col-span-2 space-y-6">
                    <div class="bg-white rounded-lg border border-slate-200 shadow-sm p-6 space-y-6">
                        <div class="space-y-1.5">
                            ...
                            <input type="text" name="title" required value="{{ old('title', $activityReport->title) }}"
                        ...
                        <div class="space-y-1.5">
                            ...
                            <textarea name="excerpt" rows="3"
                                ...
                                placeholder="Ringkasan singkat artikel yang akan muncul di daftar...">{{ old('excerpt', $activityReport->excerpt) }}</textarea>
                        </div>
                        ...
                        <div class="space-y-1.5">
                            ...
                            <input id="x" type="hidden" name="content" required
                                value="{{ old('content', $activityReport->content) }}">
                            <trix-editor input="x"
                    ...
                    <div class="bg-white rounded-lg border border-slate-200 shadow-sm p-6 space-y-4">
                        ...
                        <div class="space-y-1.5">
                            ...
                            <select name="status"
                                ...
                                <option value="draft" {{ $activityReport->status == 'draft' ? 'selected' : '' }}>Draft
                                </option>
                                <option value="published" {{ $activityReport->status == 'published' ? 'selected' : '' }}>
                                    Published</option>
                            </select>
                        </div>
                            ...
                            <select name="work_program_id" required
                                ...
                                @foreach ($programs as $prog)
                                    <option value="{{ $prog->id }}"
                                        {{ $activityReport->work_program_id == $prog->id ? 'selected' : '' }}>
                                        {{ $prog->name }}
                    ...
                    <div class="bg-white rounded-lg border border-slate-200 shadow-sm p-6 space-y-4">
                        ...
                        <div class="space-y-1.5">
                            ...
                            @if ($activityReport->image)
                                <div class="mb-3 relative rounded-lg overflow-hidden border border-slate-200">
                                    <img src="{{ asset('storage/' . $activityReport->image) }}" alt="Cover"
