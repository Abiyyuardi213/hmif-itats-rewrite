...
@section('title', 'Direktori Fungsionaris Himpunan (Masa ke Masa)')
...
@section('content')
    <div class="max-w-7xl mx-auto space-y-8">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div class="space-y-1">
                <nav class="flex items-center gap-2 text-xs font-medium text-slate-400 mb-2">
                    ...
                    <span class="text-slate-900">Direktori Pengurus</span>
                </nav>
                <h1 class="text-2xl font-bold text-slate-900 tracking-tight">Direktori Pengurus Himpunan</h1>
                <p class="text-sm text-slate-500">Database seluruh fungsionaris dan alumni pengurus HMIF ITATS dari masa ke masa.</p>
            </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div class="p-4 bg-white border border-slate-200 rounded-lg shadow-sm">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-1">Total Keseluruhan</p>
                        <p class="text-2xl font-bold text-slate-900 tracking-tight">{{ $totalMembers }}</p>
                    </div>
            ...
            <div class="p-4 bg-white border border-slate-200 rounded-lg shadow-sm">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-1">Pengurus Aktif saat ini</p>
                    ...
                    <div class="w-10 h-10 rounded-full bg-emerald-50 flex items-center justify-center text-emerald-500">
                        <i class="fas fa-[#EB329A] fa-user-check"></i>
                    </div>
                </div>
            </div>
            <div class="p-4 bg-white border border-slate-200 rounded-lg shadow-sm">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-1">Alumni Kepengurusan</p>
                        <p class="text-2xl font-bold text-slate-500 tracking-tight">
                            {{ $totalAlumni }}
                        </p>
                    </div>
        ...
        <!-- TABEL 1: PENGURUS AKTIF (PERIODE UTAMA) -->
        @if($type === 'all' || $type === 'active')
        <div class="space-y-3">
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-2">
                    <span class="w-3 h-3 rounded-full bg-emerald-500 animate-pulse"></span>
                    <h2 class="text-lg font-bold text-slate-900">Pengurus Aktif Saat Ini</h2>
                    <span class="px-2.5 py-0.5 rounded-full text-xs font-bold bg-emerald-50 text-emerald-700 border border-emerald-200">
                        {{ $activePeriod->name ?? 'Kabinet Utama' }} ({{ $activePeriod->academic_year ?? '-' }})
                    </span>
                </div>
                @if($activePeriod)
                    <a href="{{ route('admin.periods.manage_members', $activePeriod->id) }}" class="text-xs font-semibold text-primary hover:underline">
                        Kelola Struktur Kabinet Aktif →
                    </a>
                @endif
            </div>
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="bg-slate-50/50 border-b border-slate-200">
                                ...
                                <th class="px-6 py-3 text-xs font-semibold text-slate-500 uppercase tracking-tight">Kabinet</th>
                            ...
                            @forelse($activeMembers as $member)
                                <tr class="group hover:bg-slate-50/40 transition-colors">
                                    <td class="px-6 py-4">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div class="flex flex-col">
                                                ...
                                                <span class="text-[11px] text-slate-400 font-medium tracking-tight">{{ $member->npm ?? '-' }}</span>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4">
                                        <span class="inline-flex items-center px-2 py-0.5 rounded-md text-[10px] font-bold uppercase tracking-widest border {{ optional($member->position)->type == 'inti' ? 'bg-indigo-50 text-indigo-700 border-indigo-100' : 'bg-slate-50 text-slate-600 border-slate-200' }}">
                                    ...
                                    <td class="px-6 py-4">
                                        ...
                                        @else
                                            <span class="text-[11px] text-slate-300 italic">Pengurus Inti</span>
                                        @endif
                                    </td>
                                    <td class="px-6 py-4">
                                        <span class="px-2 py-0.5 rounded bg-emerald-50 text-emerald-700 text-xs font-semibold">
                                            {{ optional($member->period)->name ?? '-' }}
                                        </span>
                                    </td>
        ...
        <!-- TABEL 2: PENGURUS TIDAK AKTIF / ALUMNI MASA LALU -->
        @if($type === 'all' || $type === 'inactive')
        <div class="space-y-3 pt-4">
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-2">
                    <span class="w-3 h-3 rounded-full bg-slate-300"></span>
                    <h2 class="text-lg font-bold text-slate-900">Arsip Demisioner / Pengurus Tidak Aktif</h2>
            ...
            <div class="bg-white rounded-lg border border-slate-200 shadow-sm overflow-hidden">
                <div class="overflow-x-auto text-sm">
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="bg-slate-50/50 border-b border-slate-200">
                                <th class="px-6 py-3 text-xs font-semibold text-slate-500 uppercase tracking-tight">Demisioner</th>
                                <th class="px-6 py-3 text-xs font-semibold text-slate-500 uppercase tracking-tight">Jabatan Dulu</th>
                                <th class="px-6 py-3 text-xs font-semibold text-slate-500 uppercase tracking-tight">Divisi Dulu</th>
                                <th class="px-6 py-3 text-xs font-semibold text-slate-500 uppercase tracking-tight">Kabinet Periode</th>
                                <th class="px-6 py-3 text-xs font-semibold text-slate-500 uppercase tracking-tight text-center">Status</th>
                                <th class="px-6 py-3 text-xs font-semibold text-slate-500 uppercase tracking-tight text-right">Aksi</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-slate-100">
                            @forelse($inactiveMembers as $member)
                                <tr class="group hover:bg-slate-50/40 transition-colors">
                                    <td class="px-6 py-4">
                                        <div class="flex items-center gap-3">
                                            <div class="w-10 h-10 rounded-full bg-slate-100 overflow-hidden ring-1 ring-slate-200 flex-shrink-0 relative grayscale">
                                                @if ($member->image)
                                                    <img src="{{ asset('storage/' . $member->image) }}" class="w-full h-full object-cover opacity-80">
                                                @else
                                                    <div class="w-full h-full flex items-center justify-center text-slate-300">
                                                        <i class="fas fa-user text-lg"></i>
                                                    </div>
                                    ...
                                    <td class="px-6 py-4">
                                        <span class="inline-flex items-center px-2 py-0.5 rounded-md text-[10px] font-bold uppercase tracking-widest border bg-slate-50 text-slate-500 border-slate-200">
                                            {{ optional($member->position)->name ?? 'BPH' }}
                                        </span>
                                    </td>
                                    <td class="px-6 py-4">
                                    ...
                                    <td class="px-6 py-4">
                                        <span class="px-2 py-0.5 rounded bg-slate-100 text-slate-600 text-xs font-semibold">
                                            {{ optional($member->period)->name ?? '-' }} ({{ optional($member->period)->academic_year ?? '-' }})
                                        </span>
                                    </td>
                                    <td class="px-6 py-4 text-center">
                                        <label class="relative inline-flex items-center cursor-pointer" title="Klik untuk ubah status">
                            ...
                            @empty
                                <tr>
                                    <td colspan="5" class="px-6 py-8 text-center text-slate-400 italic">
                                        Belum ada arsip pengurus demisioner.
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
    ...
    <div id="form-modal-root" class="fixed inset-0 z-[100] hidden items-center justify-center p-4 overflow-y-auto">
        ...
        <div id="form-content"
            ...
            <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                <h3 id="form-modal-title" class="font-semibold text-slate-900">Kelola Fungsionaris</h3>
            ...
            <form id="member-form" action="{{ route('admin.members.store') }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-4">
                ...
                <div class="space-y-1.5">
                    <label class="text-xs font-semibold text-slate-700 uppercase tracking-wider">Kabinet / Periode <span class="text-rose-500">*</span></label>
                    <select name="period_id" id="f-period_id" required
                        ...
                        @foreach ($periods as $p)
                            <option value="{{ $p->id }}">{{ $p->name }} ({{ $p->academic_year }}) {{ $p->is_active ? '— (Kabinet Aktif)' : '' }}</option>
                        @endforeach
                    </select>
                </div>
    ...
    <script>
        ...
        function openCreateModal() {
            ...
            document.getElementById('form-modal-title').innerText = 'Tambah Fungsionaris';
            document.getElementById('submit-btn').innerText = 'Simpan';
            document.getElementById('f-period_id').value = '{{ $activePeriod->id ?? "" }}';
        ...
        function openEditModal(m) {
            ...
            document.getElementById('form-modal-title').innerText = 'Edit Fungsionaris';
            document.getElementById('submit-btn').innerText = 'Update';

            document.getElementById('f-period_id').value = m.period_id || '';
        ...
        function confirmDelete(id, name) {
            Swal.fire({
                ...
                text: `"${name}" akan dihapus dari direktori.`,
                icon: 'warning',
