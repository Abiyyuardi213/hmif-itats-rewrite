...
@section('content')
    <div class="max-w-2xl mx-auto">
        <div class="mb-8">
            ...
            <h1 class="text-3xl font-bold text-slate-900 tracking-tight">Edit Jabatan</h1>
            <p class="text-slate-500 mt-1">Perbarui informasi jabatan.</p>
        </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-2">
                    ...
                    <select name="type" required
                        class="w-full h-12 px-4 rounded-xl border border-slate-200 bg-slate-50/50 focus:bg-white focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-all">
                        <option value="inti" {{ old('type', $position->type) == 'inti' ? 'selected' : '' }}>Pengurus Inti
                        </option>
                        <option value="divisi" {{ old('type', $position->type) == 'divisi' ? 'selected' : '' }}>Divisi
                        </option>
                    </select>
                </div>
