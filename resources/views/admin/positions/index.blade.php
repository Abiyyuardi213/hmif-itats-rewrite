...
@section('content')
    <div class="space-y-6">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-3xl font-bold text-slate-900 tracking-tight">Data Jabatan</h1>
                <p class="text-slate-500 mt-1">Kelola daftar jabatan yang tersedia dalam organisasi.</p>
            </div>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-slate-50/50 border-b border-slate-100">
                        <th class="px-6 py-4 text-xs font-bold uppercase tracking-widest text-slate-400">Nama Jabatan</th>
                        <th class="px-6 py-4 text-xs font-bold uppercase tracking-widest text-slate-400">Tipe</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse($positions as $position)
                        <tr class="hover:bg-slate-50/50 transition-colors">
                            <td class="px-6 py-4 font-bold text-slate-900 text-sm">{{ $position->name }}</td>
                            <td class="px-6 py-4">
                                <span
                                    class="px-2 py-1 rounded text-[10px] font-bold border {{ $position->type == 'inti' ? 'bg-indigo-50 text-indigo-700 border-indigo-100' : 'bg-amber-50 text-amber-700 border-amber-100' }} uppercase">
                                    {{ $position->type }}
                                </span>
                            </td>
                            <td class="px-6 py-4 text-center text-slate-500 text-sm">{{ $position->order }}</td>
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="px-6 py-12 text-center text-slate-400 italic">Belum ada data jabatan.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
